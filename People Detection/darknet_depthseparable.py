...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            total_loss = 10*(loss_x + loss_y + loss_w + loss_h) + 2*(loss_conf) + loss_cls
            if self.debug:
	...
	def __init__(self, debug=False):
		...
		def conv_depthwise(inp, oup, stride):
			return nn.Sequential(
				...
				nn.BatchNorm2d(inp, momentum=0.9, eps=1e-5),
				nn.LeakyReLU(.1),
				nn.Conv2d(inp, oup, 1, 1, 0, bias=False),
				nn.BatchNorm2d(oup, momentum=0.9, eps=1e-5),
				nn.LeakyReLU(.1)
			)
		...
		self.conv1 = nn.Sequential(nn.Conv2d(1, 16, kernel_size = 3, stride=1, padding=1, bias=False),
								   self.bn1,
								   nn.LeakyReLU(.1),
								   
				  				  )

		self.conv2 = conv_depthwise(16, 32, 1)
								  	  				  
		self.conv3 = conv_depthwise(32, 64, 1)

		self.conv4 = conv_depthwise(64, 128, 1)

		self.conv5 = conv_depthwise(128, 256, 1)

		# self.conv6 = conv_depthwise(256, 512, 1)

		# self.conv7 = conv_depthwise(512, 1024, 1)


		#pathway one

		self.conv8p1 = nn.Sequential(nn.Conv2d(256, 256, kernel_size = 1, stride=1, bias=False),
								   self.bn8p1,
								   nn.LeakyReLU(.1),
				  				  )
		self.conv9p1 =  conv_depthwise(256, 512, 1)
		...
		self.conv8p2 = conv_depthwise(256, 128, 1)

		self.conv9p2 = conv_depthwise(256, 256, 1)

		self.conv10p2 = nn.Sequential(nn.Conv2d(256, 18, kernel_size = 1, stride=1, bias=False),
				  				  )

		self.yolo2 = YOLOLayer(anchors=[(23,27), (37,58), (81,82)], num_classes=1, debug=debug)
